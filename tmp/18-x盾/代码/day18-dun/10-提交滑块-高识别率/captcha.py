...
def get_conf():
    ...
    res = requests.get(url="https://c.dun.163.com/api/v2/getconf",
                       params={"referer": "https://lhqkl.ydmap.cn/user/login", "zoneId": "",
                               "id": "0908c3b5498d40ed8e17328b88a7d6a9", "ipv6": "false", "runEnv": "10", "iv": "4",
                               "type": "2", "loadVersion": "2.4.0", "callback": callback_string},
...
def get(dt, ac_bid, ac_token, cb, fp):
    ...
    res = requests.get(url="https://c.dun.163.com/api/v3/get",
                       params={"referer": "https://lhqkl.ydmap.cn/user/login", "zoneId": "CN31", "dt": dt,
                               "acToken": ac_token, "id": ac_bid, "fp": fp, "https": "true", "type": None,
                               "version": "2.27.2", "dpr": "1", "dev": "1", "cb": cb, "ipv6": "false", "runEnv": "10",
                               "group": "", "scene": "", "lang": "zh-CN", "sdkVersion": "undefined", "iv": "4",
                               "width": "220" if first else '440', "audio": "false", "sizeType": "10",
...
def run():
    ...
    m_string = ""
    p_string = j_as_ww(N_as_w8(bg_token, str(x_distance / (320 if first else 480) * 100)))
    ...
    params = {"referer": "https://lhqkl.ydmap.cn/user/login", "zoneId": bg_zone_id, "dt": dt, "id": ac_bid,
        ...
        "version": "2.27.2", "cb": cb, "extraData": "", "bf": "0", "runEnv": "10", "sdkVersion": "undefined", "iv": "4",
        "callback": callback_string}
    res = requests.get(url="https://c.dun.163.com/api/v3/check", params=params,
...
if __name__ == '__main__':
    ...
    while True:
        res = run()
        if res['msg'] == 'ok' and res['data']['result']:
            print(res)
            print("验证成功")
            break
        else:
